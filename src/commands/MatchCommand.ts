...
export default class MatchCommand extends Command implements ApplicationCommand {
    ...
    public async execute(interaction: CommandInteraction): Promise<void> {
        ...
        try {
            await fetch(`https://api.henrikdev.xyz/valorant/v2/match/${matchId}?api_key=HDEV-04d0ed17-947a-49c0-871a-41ca3314250d`)
                ...
                .then(async res => {
                    ...
                    const embed: MessageEmbed = new MessageEmbed()
                        .setAuthor({name: `Match Data: ${data.metadata.mode} [${data.metadata.region.toUpperCase()}]`, iconURL: data.players.all_players[0].assets.agent.small})
                        .setColor(CypherNetworkConstants.DEFAULT_EMBED_COLOR)
                        .setDescription(`• Map: **${data.metadata.map}**` + `\n`
            ...
            console.log(e)
            const embed: MessageEmbed = EmbedUtil.getErrorEmbed("An error occurred while match competitive data.");
            return void await interaction.editReply({embeds: [embed]});
        }
    }
